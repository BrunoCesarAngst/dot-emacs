...
* ~Aqui eu gerencio o sh do sistema~
#+begin_src shell :comments no :tangle ~/.testrc
  # _*_ mode: sh; _*_

  export EMACS_DIR=~/.emacs.d
#+end_src
...
Este trecho acrescenta um gancho a =org-mode= buffers de modo que =bca/org-babel-tangle-config= seja executado cada vez que um tal buffer é salvo.	Esta função verifica se o arquivo que está a ser guardado é o ficheiro config.org que está a ver neste momento, e se assim for, exporta automaticamente a configuração aqui para os ficheiros de saída associados.
...
O Emacs tem um gerenciador de pacotes integrado, mas não facilita a instalação automática de pacotes em um novo sistema na primeira vez que você baixa sua configuração.  [[https://github.com/jwiegley/use-package][use-package]] é um pacote realmente útil utilizado nesta configuração para tornar muito mais fácil automatizar a instalação e configuração de tudo o mais que usamos.
...
Bob -> Margarete : Hello World!
#+end_src

#+RESULTS:
[[file:~/hello-uml.png]]

* Características IDE para desenvolvimento
** Rodar código:
...
* Pacotes personalizados que melhoram o EMACS UX -
